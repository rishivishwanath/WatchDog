...
async def process_l1_bbo(exchange:str,symbol:str,bid_price:float,bid_size:float,ask_price:float,ask_size:float,timestamp:str):
    ...
    asyncio.create_task(compare(bid_price,ask_price,symbol,exchange, bid_size,ask_size, timestamp, all_exchange_data))
    

async def compare(bid_price:float,ask_price:float,symbol:str, exchange_prior:str,bid_size:float,ask_size:float,timestamp:str,all_exchange_data):
    for exchange, data in all_exchange_data[1].items():
        if exchange == exchange_prior:
            continue
